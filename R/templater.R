...
templater <- function(date_chr, project_name, readme = TRUE) {
  ...
  # --- [ EXISTING CODE FOR 01_IMPORT ] ---
  r_path_1 <- system.file(
    "r-template.R",
    package = "templater",
    mustWork = TRUE
  )
  r_txt_1 <- readLines(r_path_1)
  # (Apply your gsubs here for r_txt_1)
  writeLines(
    r_txt_1,
    file.path(folder_name, "scripts", "01_import_and_clean.R")
  )

  # --- [ NEW CODE FOR 02_GRAPHING ] ---
  r_path_2 <- system.file(
    "graphing-template.R",
    package = "templater",
    mustWork = TRUE
  )
  r_txt_2 <- readLines(r_path_2)

  # Apply replacements to the second file
  r_txt_2 <- gsub("{{title}}", project_name, r_txt_2, fixed = TRUE)
  r_txt_2 <- gsub("{{date_chr}}", date_chr, r_txt_2, fixed = TRUE)

  writeLines(r_txt_2, file.path(folder_name, "scripts", "02_graphing.R"))

  # 5. Process the README
  if (readme) {
    readme_path <- system.file(
      "readme-template.md",
      package = "templater",
      mustWork = TRUE
    )
